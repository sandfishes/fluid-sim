...
// Compiles the triangle.slang shader and creates the pipeline (or re-creates it if it exists)
compile_compute_shader_module :: proc(path, main: cstring) -> vk.ShaderModule
{
	// time the compilation
	start_compile_time: time.Tick = time.tick_now()
	...
	entry: ^sl.IEntryPoint
	r = module->findEntryPointByName(main, &entry)
	slang_check(r)
	...
	components: [2]^sl.IComponentType = {module, entry}
